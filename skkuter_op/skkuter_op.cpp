...
torch::Tensor attention_forward(
    ...
    bool use_cuda_kernel = true;

    if(use_cuda_kernel){

        if (!attention_mask.defined()){
            //create a dummy attention mask with zeros
            attention_mask = torch::zeros({bsz, 1, q_len, kv_seq_len}, query_states.options());
        }

        //cast them to float32
        query_states = query_states.to(torch::kFloat32);
        key_states = key_states.to(torch::kFloat32);
        value_states = value_states.to(torch::kFloat32);
        attention_mask = attention_mask.to(torch::kFloat32);
        
        auto attn_weights = cuda_attn_forward(query_states, key_states, value_states, attention_mask).to(torch::kFloat16);
        auto attn_output = torch::nn::functional::dropout(attn_weights, torch::nn::functional::DropoutFuncOptions().p(attention_dropout));
